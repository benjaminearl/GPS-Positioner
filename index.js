...
console.log(satID.length);

// async function jsonParse (satellite) {
//     const res = await fetch(`https://www.n2yo.com/rest/v1/satellite/positions/${satID[satellite]}/52.070499/4.300700/0/1/&apiKey=${apiConfig.satelliteKey}`);
         

//     const json = await res.json();
//     const location = json.positions[0].satlatitude+","+json.positions[0].satlongitude;
//     const satName = json.info.satname;
//     // console.log(satName + " @ " + location);


//     const meta = await fetch(`https://maps.googleapis.com/maps/api/streetview/metadata?size=600x300&location=${location}&pitch=90&key=${apiConfig.streetViewKey}`)
//     const metadata = await meta.json();
    
//     if(metadata.status === "ZERO_RESULTS"){ 
//         console.log("No image found " + satName)
//         return;
//     } else {
//         const image = await fetch(`https://maps.googleapis.com/maps/api/streetview?size=600x600&location=${location}&pitch=90&key=${apiConfig.streetViewKey}`);
//         const dest = fs.createWriteStream(`images/${satName} - ${location}.jpg`);
